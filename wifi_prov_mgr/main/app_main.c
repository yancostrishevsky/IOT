...
void dht_test(void *pvParameters) {
  ...
  while (1) {
    if (dht_read_float_data(SENSOR_TYPE, DHT_GPIO_PIN, &humidity,
                            &temperature) == ESP_OK) {
      char message[100];
      snprintf(message, sizeof(message), "Temperatura: %.1f, Wilgotnosc: %.1f",
               temperature, humidity);
      char temperatureMessage[50];
      char humidityMessage[50];

      snprintf(temperatureMessage, sizeof(temperatureMessage), "%.1f",
               temperature);
      ...
      snprintf(humidityBuffer, sizeof(humidityBuffer), "Wilg.: %.1f%%",
               humidity);
      ssd1306_display_text(&dev, 0, humidityBuffer, strlen(humidityBuffer),
                           false);
      char temperatureBuffer[32];
      snprintf(temperatureBuffer, sizeof(temperatureBuffer), "Temp.: %.1fC",
               temperature);
      ssd1306_display_text(&dev, 1, temperatureBuffer,
                           strlen(temperatureBuffer), false);
...
void app_main(void) {
  ...
  xTaskCreate(ultrasonic_sensor_task, "ultrasonic_sensor_task", 4096, NULL, 5,
              NULL);
  ...
  if (!provisioned) {
    ...
    ESP_ERROR_CHECK(wifi_prov_mgr_start_provisioning(
        security, (const void *)pop, PROVISIONING_PREFIX, NULL));
  } else {
